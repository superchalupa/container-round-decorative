...
module generic_container(box_height, radius, minor_radius, wall_thick, bottom_thick, spiro_steps, spiro_line_width, hole_len, distance_between_holes, hole_rotation_angle, num_divisions_around) 
{
    union() {
        ...
        difference() {
            ...
            translate([0,0,box_height-bottom_thick-fitting_windage_snug])
...
module container_with_latches(box_height, radius, minor_radius, wall_thick, bottom_thick, spiro_steps, spiro_line_width, hole_len, distance_between_holes, hole_rotation_angle, num_divisions_around)
{
    difference() {
        ...
        translate([0,0,box_height])
            rotate([180,0,0])
            generic_container_lid_with_latches(box_height=box_height,
                  radius=radius,
                  minor_radius=minor_radius,
...
module container_with_pin(box_height, radius, minor_radius, wall_thick, bottom_thick, spiro_steps, spiro_line_width, hole_len, distance_between_holes, hole_rotation_angle, num_divisions_around) 
{
    ...
    union() {
        difference() {
            ...
            translate([0,0,box_height])
                rotate([180,0,0])
                generic_container_lid_with_pin(box_height=box_height,
                      radius=radius,
                      minor_radius=minor_radius,
                      wall_thick=wall_thick,
                      bottom_thick=bottom_thick,
...
module detents(radius, wall_thick)
{
        // detents
        translate([0, (radius-wall_thick/2+smidgen), 0.5])
            cube( [ wall_thick,  wall_thick, 1], center=true);
        translate([0,-(radius-wall_thick/2+smidgen), 0.5])
            cube( [ wall_thick,  wall_thick, 1], center=true);
}
