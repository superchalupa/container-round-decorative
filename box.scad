...
module container(box_height, radius, wall_thick, bottom_thick, spiro_steps) {
    oval_maj_rad=6;
    distance_between=1;
    rotate_angle = 60;
    num_around = 22;
    y_step = sin(rotate_angle) * (oval_maj_rad*2+distance_between);
    num_big_ovals = floor((box_height-(bottom_thick+1)*2)/y_step);
    ...
    union() {
        ...
        difference() {
	        difference() {
                ...
                translate([0,0,box_height/2-bottom_thick]) cylinder(h=10,r=radius+10);
	        }
        ...
        spiro(radius/2 + 0.5, lineWidth, bottom_thick, spiro_steps);
        cylinder(h=1,r=radius+1);
        tube(radius+1, lineWidth, bottom_thick);
...
module container_lid(box_height, radius, bottom_thick, spiro_steps) {
    union() {
        translate([0,0,box_height/2])
        difference() {
...
if (layout=="preview"){
    container(box_height=box_height, radius=radius, wall_thick=wall_thick, bottom_thick=bottom_thick, spiro_steps=spiro_steps);
    #translate([0,0,box_height])
        rotate([180,0,0])
            container_lid(box_height=box_height, radius=radius, bottom_thick=bottom_thick, spiro_steps=spiro_steps);
}
